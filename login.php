...
// Ambil Data Profil Toko dengan pengecekan aman
$nama_toko = 'POS Kasir Pro';
$alamat_toko = 'Silakan atur profil toko';
$logo_toko = '';
...
if ($q_toko && mysqli_num_rows($q_toko) > 0) {
    ...
    $nama_toko = !empty($toko['store_name']) ? $toko['store_name'] : $nama_toko;
    $alamat_toko = !empty($toko['address']) ? $toko['address'] : $alamat_toko;
    $logo_toko = $toko['logo'] ?? '';
}
...
if (isset($_POST['login'])) {
    ...
    if (mysqli_num_rows($query) > 0) {
        ...
        // Buka Shift
        $user_id = $user['id'];
        mysqli_query($conn, "UPDATE shifts SET status = 'closed', end_time = NOW() WHERE user_id = $user_id AND status = 'active'");
        mysqli_query($conn, "INSERT INTO shifts (user_id, status) VALUES ($user_id, 'active')");
